const views = (function () {
    ...
    return {
            ...
            name.forEach(i => {
                ...
                if (data[i].size > 0 || data[i].length > 0) {
                    ...
                    for (const [id, obj] of data[i].entries()) {
                        ...
                        const valueType = obj.type === 'expense' ? '- ' + obj.value.toLocaleString() : '+ ' + obj.value.toLocaleString();
                        ...
                        <div class="${obj.type}" data-id="${id}" data-date="${Number(date)}">
                            ...
                            <span class="del__btn far fa-times-circle"></span>
                        </div>
                        `;
                    }
        ...
        updateTotal(data) {
            const balance = (data.balance % 1) === 0 ? data.balance.toLocaleString() + '.00' : data.balance.toLocaleString();
...
const models = (function () {
    ...
    return {
        ...
        async fetchFn(year) {
            ...
            try {
                return await fetch(`http://localhost:3000/${year}`)
                    .then(rows => {
                        return rows.json()
                    })
                    .then(res => {
        ...
        async putApi(curData, year, month) {
            ...
            try {
                await fetch(`http://localhost:3000/${year}/${month}`, {
                    method: 'put',
                    headers: {
...
const controller = (function (model, UI) {
    ...
    // Change month FETCH API
    let prevMonth = 0;
    UI.elem.date.addEventListener('change', e => {
        // e.preventDefault();
    ...
    return {
        async init() {
            const [curYear, curMonth, curDay] = [new Date().getFullYear(), new Date().getMonth() + 1, new Date().getDate()]
            // SET CURRENT DATE TO DATE INPUT
            UI.elem.date.value = [curYear, curMonth, curDay].join('-').toString();

            // FETCH API DATA 
            // PUT API DATA TO MAP DATA
            // UPDATE TOTAL DATA
            model.fetchFn(curYear - 1)
